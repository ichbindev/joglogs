...
function createCalendar(calendarInfo, cb) {
  ...
  if (calendarInfo.whichGoogleAccountInteger < 1 || calendarInfo.whichGoogleAccountInteger > 2) { calendarInfo.whichGoogleAccountInteger = 1; }
  ...
  function insertCalendar(auth) {
    console.log("Trying to Create/Insert New Calendar, Named = " + calendarInfo.summary);
    ...
    calendar.calendars.insert(
      ...
      function (err, res) {
        ...
        console.log("\n\nSuccessfully created Calendar Named : " + res.data.summary + "\n  with id # " + res.data.id + "\n and etag: " + res.data.etag);
  ...
  function shareCalendar(auth, calendarInfo) {
    if (calendarInfo.runnersGoogleEmail) {
      console.log("Now trying to share Calendar with: " + calendarInfo.runnersGoogleEmail);
    } else {
      calendarInfo.runnersGoogleEmail = "aaroncentex@gmail.com";
      console.log("calendarInfo.runnersGoogleEmail is blank, so using " + calendarInfo.runnersGoogleEmail);
    }
    const calendar = google.calendar({ version: 'v3', auth });
    calendar.acl.insert(
      {
      ...
      ,
      (err, res) => {
        if (err) return console.log('Calendar API failed on calendar#aclRule: ' + err);

        console.log("Success sharing Calendar using calendar#aclRule to: " + calendarInfo.runnersGoogleEmail)
  ...
  function insertEvent(auth, calendarInfo) {
    ...
    for (var i = 0; i < 180; i++) {
      ...
      if (theDay === "Sat") {
        ...
        makeEvent.thisEventdescription = "Big " + calendarInfo.summary + " Saturday Run! " + Math.round(milesToday) + " Mile Run Today. " + Math.round(weeksLeft) + " Weeks of Training Left for Marathon : " + calendarInfo.summary + "!";
        ...
        makeEvent.thisEventdescription = "Thursday " + calendarInfo.summary + " Fun Run! " + Math.round(milesToday) + " Mile Run Today. " + Math.round(weeksLeft) + " Weeks of Training Left for Marathon : " + calendarInfo.summary + "!";
        ...
        makeEvent = [];
        milesToday = (endingMiles / 6);
        makeEvent.thisEventdescription = "Tuesday Jog for " + calendarInfo.summary + " ! " + Math.round(milesToday) + " Mile Run Today. " + (weeksLeft) + " Weeks of Training Left for Marathon : " + calendarInfo.summary + "!";
        ...
        makeEvent = [];
        milesToday = (endingMiles / 6);
        makeEvent.thisEventdescription = "Monday Marathon Training! " + Math.round(milesToday) + " Mile Run Today. " + Math.round(weeksLeft) + " Weeks of Training Left for Marathon : " + calendarInfo.summary + "!";
      ...
      if (eventDate <= new Date()) {
        ...
        console.log("\n\n Total Number of Events To Add to Calendar = " + makeEvents.length);
        ...
        function methodThatReturnsAPromise(anEvent) {
          return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
              ...
              addThisEvent(anEvent).then(function (result) {
                resolve();
              })
    ...
    function addThisEvent(makeEvent) {
      ...
      return new Promise(function (resolve, reject) {
        calendar.events.insert({
          ...
          if (err) {
            console.log('calender.events.insert failed in Calendar service: ' + err);
            reject(err)
          }
          eventCount++
