...
function marathonSchedule(data) {
    ...
    calculateStartDate = (new Date(calculateStartDate).toJSON().substr(0, 10) + "T12:00:00");

    const raceDate = new Date().toJSON().substr(0, 10);
    let runnerData = [];
    let day = [];
   
    if (data === undefined) {
        logthis("ERROR! ***** data for runner was undefined, using sample data *****");
        // setup sample data
    ...
    let runsPerWeek = 0;
    let runDays = [];
    if (runnerData.runSunday) { runsPerWeek++; runDays.push(0); };
    if (runnerData.runMonday) { runsPerWeek++; runDays.push(1); };
    if (runnerData.runTuesday) { runsPerWeek++; runDays.push(2); };
    if (runnerData.runWednesday) { runsPerWeek++; runDays.push(3); };
    if (runnerData.runThursday) { runsPerWeek++; runDays.push(4); };
    if (runnerData.runFriday) { runsPerWeek++; runDays.push(5); };
    if (runnerData.runSaturday) { runsPerWeek++; runDays.push(6); };
    ...
    let lastRegularTrainingDay = new Date(runnerData.raceDate + "T12:00:00").getTime() - (24 * 60 * 60 * 1000 * 14) //= 14 days
    logthis("lastRegularTrainingDay (before taper) = " + new Date(lastRegularTrainingDay).toJSON().substr(0, 10));
    ...
    logthis("startDate in ms = " + runnerData.startDate + " = " + new Date(runnerData.startDate));

    runnerData.raceDate = new Date(runnerData.raceDate + "T12:00:00").getTime();
    logthis("raceDate in ms = " + runnerData.raceDate + " = " + new Date(runnerData.raceDate));
    ...
    if (runnerData.startMilesPerWeek + (weeksToTrain * weeklyIncrement) > maxMilesPerWeek) {
        // Compute new weeklyIncrement to reach max miles per week in the number of weeks of training. 
        weeklyIncrement = ((maxMilesPerWeek - runnerData.startMilesPerWeek) / weeksToTrain)
    }
    logthis("weeklyIncrement adjusted down if runner exceeds maxMilesPerWeek = " + weeklyIncrement);
    logthis("The peak miles per week will reach " + ((weeklyIncrement * weeksToTrain) + (runnerData.startMilesPerWeek)));
    ...
    logthis("run days (computed from Sunday) array:"); logthis(day);
...
let scheduleWeekStart = new Date(todayNoon).setDate(new Date().getDate() - new Date(todayNoon).getDay() +1); 
logthis("last Sunday = "+ new Date(scheduleWeekStart))
let tempEventDate= new Date().getTime();
logthis("tempEventDate = "+tempEventDate+"\nlastRegularTrainingDay = "+lastRegularTrainingDay);
...
tempEventDate = scheduleWeekStart +(runDays[i] * dayOfMilliseconds )+(7 * weekNumber * dayOfMilliseconds)  ;
...
event.milesToRunToday = Math.round(  (runnerData.startMilesPerWeek + (weeklyIncrement * weekNumber)) * ( day[i].percentMilesPerWeek / 100 )  *10) /10  ;
event.mileTotalThisWeek = Math.round( (runnerData.startMilesPerWeek + (weeklyIncrement * weekNumber)) *10) /10;
